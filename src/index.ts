...
4. Lead by delegation: send guidance with agent_teams_send_message and let idle teammates execute ready work. Do not duplicate a teammate's work merely because its turn is slow. Members report completion to you automatically via agent_teams_send_message — you receive their messages as inbox deliveries, so do NOT poll agent_teams_status in a loop. Call agent_teams_status at most ONCE after creating tasks to confirm dispatch, then wait for member messages. If you must wait for long-running work, use bash sleep (e.g. sleep 30) instead of repeatedly calling status.
